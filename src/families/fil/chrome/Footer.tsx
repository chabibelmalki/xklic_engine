...
/**
 * FOOTER fil — ÉTIQUETTE COUSUE en pied de site : pan d'encre `brand-800`
 * refermé par la couture signature (fil pointillé + nœud), wordmark serif
 * (Fraunces), colonnes à titres MAJUSCULES, hover sable. Props identiques à
 * `SiteFooter` (contrat de famille) ; obligations conservées : liens légaux,
 * page avis, backlink `xklic.com`, copyright + identifiant légal. AA garanti
 * (clairs sur encre).
 */
export function FilFooter({
  config,
  basePath,
  ...
  return (
    <footer className="relative bg-brand-800 text-white">
      {/* Couture de tête : fil pointillé + nœud central. */}
      <div aria-hidden className="relative">
        <span className="fil-seam absolute inset-x-0 top-0 h-px text-accent-500/70" />
        <span className="absolute left-1/2 top-[-4px] size-2.5 -translate-x-1/2 rounded-full border border-accent-500 bg-brand-800" />
      </div>
      <div aria-hidden className="fil-grain absolute inset-0 text-white/50" />
      <FilContainer wide className="relative py-16 sm:py-20">
        <div className="grid gap-12 lg:grid-cols-[1.6fr_1fr_1.2fr]">
          {/* Marque — wordmark serif (cohérent avec l'encre du pied). */}
          <div>
            <Link href={basePath || "/"} aria-label={e.nom} className="inline-flex flex-col leading-none">
              <span className="font-display text-2xl tracking-tight text-white">{e.nom}</span>
