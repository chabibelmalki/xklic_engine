...
import { MeridianField } from "../ui/Escale";

/**
 * FOOTER escale — le POSTE DE NUIT : panneau brand-800 au graticule de méridiens,
 * intitulés de colonne en mono capitales, coordonnées en chiffres tabulaires.
 *
 * Conserve TOUTES les obligations du contrat : navigation résolue, réseaux
 * sociaux, liens légaux (mentions + confidentialité), page avis conditionnée à
 * `googleReviewUrl`, backlink SEO `xklic.com`, copyright + identifiant légal.
 * Props identiques à `SiteFooter`.
 */
export function EscaleFooter({
  config,
  basePath,
  ...
  const heading = (label: string) => (
    <h3 className="escale-mono text-[0.7rem] font-bold text-accent-500">{label}</h3>
  );
    ...
    <footer className="relative isolate overflow-hidden bg-brand-800 text-white">
      ...
      <EditorialContainer className="relative py-16 sm:py-20">
        <div className="grid gap-12 lg:grid-cols-[1.6fr_1fr_1.2fr]">
          {/* Marque — wordmark rendu en blanc (variant light) sur le panneau de nuit. */}
          <div>
            ...
            {socials.length > 0 && (
              <SocialLinks
                ...
                linkClassName="size-9 rounded-[var(--radius-btn)] border border-white/25 text-white hover:border-accent-500 hover:bg-accent-500 hover:text-accent-contrast"
              />
            )}
          </div>
          ...
          <div>
            ...
            <ul className="mt-5 space-y-3 text-sm text-white/75">
              ...
              {contact?.telephone && (
                <li>
                  <a href={telHref(contact.telephone)} className="escale-mono font-bold hover:text-white">
                    {contact.telephone}
                  </a>
                </li>
              )}
