...
/**
 * CTA littoral : BANDE NUIT MARINE immersive (le logo en clôture de page) —
 * navy `brand-800`, soleil doré (`Sunburst`), filet doré en tête, grand titre
 * Cormorant, CTA OR. Grammaire volontairement OPPOSÉE aux sections claires du
 * corps → moment fort. Couleurs 100 % tokens (brand/accent = palette client).
 */
export function Cta({ block, basePath = "" }: BlockComponentProps<CtaContent>) {
  ...
  return (
    <section className="relative isolate overflow-hidden bg-brand-800 text-white">
      <LittoralSunburst position="top-center" />
      <LittoralContainer wide className="relative z-10 py-20 sm:py-28">
        <div className="mx-auto max-w-3xl text-center">
          <div className="mx-auto mb-8 h-px w-16 bg-accent-500" />
          <h2 className="font-display text-3xl font-semibold leading-[1.06] tracking-[-0.01em] sm:text-5xl">
            {c.titre}
          </h2>
          {c.sousTitre && (
