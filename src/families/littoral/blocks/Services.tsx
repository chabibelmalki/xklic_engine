...
import { LittoralImage } from "../ui/Image";

/**
 * SERVICES littoral. Trois grammaires (héritées de la logique editorial mais
 * habillées « bord de mer ») :
 *  - photos majoritaires → GRILLE PHOTO magazine, filet doré au-dessus du titre.
 *  - items avec `href` → CARTES ÉCUME (fond pâle, liseré marine à gauche, gros
 *    numéro serif doré, icône en pastille marine, prix + CTA en pied).
 *  - sinon → LISTE numérotée à filets, grands numéros serif dorés.
...
export function Services({
  ...
  return (
    <LittoralSection id="services" tone={tone}>
      <LittoralContainer wide>
        <LittoralHeading kicker={c.eyebrow} title={c.titre ?? ""} lede={c.intro} />
        ...
        {withImages ? (
          <div className="mt-14 grid gap-x-10 gap-y-14 sm:grid-cols-2 lg:grid-cols-3">
            {items.map((s) => (
              <article key={s.nom} className="group">
                ...
                <div className="mt-5">
                  <div className="mb-3 h-px w-10 bg-accent-500" />
          ...
          <div className="mt-14 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
            {items.map((s, i) => (
              <article
                key={s.nom}
                className="flex h-full flex-col rounded-[var(--radius-card)] border border-brand-100 border-s-[3px] border-s-brand-600 bg-[color-mix(in_srgb,var(--brand-50)_45%,var(--bg))] p-7"
              >
                <div className="flex items-start justify-between gap-3">
                  <span className="font-display text-3xl font-semibold tabular-nums text-accent-600">
                ...
                {s.icone && (
                  <span className="mt-4 grid size-11 place-items-center rounded-xl bg-brand-600 text-brand-contrast">
          ...
          <div className="mt-14 border-t border-brand-100">
            {items.map((s, i) => (
              ...
              >
                <div className="font-display text-3xl font-semibold tabular-nums text-accent-600 sm:text-4xl">
                  {String(i + 1).padStart(2, "0")}
                </div>
                <div>
