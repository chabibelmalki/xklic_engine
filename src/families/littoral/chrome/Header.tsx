...
/**
 * HEADER littoral — SERVER pur : barre CLAIRE et aérée posée au-dessus de la nuit
 * marine du hero (fort contraste net). Filet doré fin en pied, nav MAJUSCULE
 * espacée avec SOULIGNÉ OR sur la page active, menu mobile natif `<details>`
...
export function LittoralHeader({
  config,
  basePath,
  currentPath = "/",
  locale,
    ...
    <header className="sticky top-0 z-50 border-b border-brand-100 bg-bg/95 backdrop-blur-sm supports-[backdrop-filter]:bg-bg/80">
      <div className="mx-auto flex h-20 w-full max-w-7xl items-center justify-between gap-8 px-6 sm:h-24 sm:px-10">
          ...
          {nav.map((item) => (
            <a
              ...
              className={cn(
                ...
                item.active
                  ? "text-brand-800 after:absolute after:inset-x-0 after:-bottom-0.5 after:h-0.5 after:bg-accent-500"
        ...
        <details className="group lg:hidden [&_.open-i]:block [&_.close-i]:hidden [&[open]_.open-i]:hidden [&[open]_.close-i]:block">
          ...
          <div className="absolute inset-x-0 top-full border-b border-brand-100 bg-bg">
