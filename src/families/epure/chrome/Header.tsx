...
/**
 * HEADER épure — SERVER pur : fin liseré de marque en TÊTE (pleine largeur), puis
 * barre claire en TROIS colonnes (logo à gauche, nav CENTRÉE, actions à droite).
 * État actif = pastille encadrée. Menu mobile natif `<details>` (zéro JS). Cette
 * structure (nav centrée + liseré haut) est propre à la famille — distincte
 * d'editorial (nav à droite, filet bas) et de classic. Props = contrat de
    ...
    <header className="sticky top-0 z-50 bg-bg/95 backdrop-blur-sm supports-[backdrop-filter]:bg-bg/80">
      ...
      <div className="border-b border-border">
        <div className="mx-auto grid h-16 w-full max-w-7xl grid-cols-[minmax(0,auto)_1fr_auto] items-center gap-4 px-5 sm:h-20 sm:px-8 lg:px-10">
          ...
          {/* Colonne 2 : nav CENTRÉE (desktop) */}
          <nav className="hidden items-center justify-center gap-x-1 lg:flex">
            {nav.map((item) => (
              <a
                key={item.href}
                href={item.href}
