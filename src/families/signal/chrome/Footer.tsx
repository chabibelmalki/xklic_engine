...
/**
 * FOOTER signal — CLAIR et structuré : coiffé d'un FILET de marque, colonnes à
 * intitulés en étiquettes (carré d'accent + majuscules espacées), réseaux en
 * puces CARRÉES à filet. Conserve les obligations (liens légaux + avis + backlink
 * `xklic.com` pour le SEO). Props identiques à `SiteFooter`.
 */
export function SignalFooter({
  ...
  const heading = (label: string) => (
    <h3 className="flex items-center gap-2 text-[0.72rem] font-bold uppercase tracking-[0.2em] text-muted">
      <span aria-hidden className="size-2 shrink-0 bg-accent-500" />
      {label}
    </h3>
  );

  return (
    <footer className="border-t-2 border-brand-500 bg-alt text-ink">
          ...
          <div>
            ...
            {socials.length > 0 && (
              <SocialLinks
                ...
                linkClassName="size-9 border border-border text-ink-soft hover:border-brand-300 hover:bg-surface hover:text-brand-700"
              />
            )}
          </div>
