...
import { withBase, telHref } from "@/lib/utils";
...
/**
 * HERO cascade — IMMERSIF & LUMINEUX : grand dégradé bleu→vert (jour), halos
 * d'eau flous, gouttelettes, titre display géométrique blanc, CTA en pilules, et
 * — signature de la famille — un SCEAU circulaire + une CARTE DE VERRE GIVRÉ à
 * droite. Le bloc se termine par une VAGUE qui déverse le dégradé dans la page.
 * Volontairement à l'opposé du hero nuit-marine-serif de `littoral`. 100 % tokens.
 */
// Dégradé signature (bleu profond → vert frais). Assez sombre pour garantir l'AA
// du texte blanc, quelle que soit la palette client (dérivée de branding.colors).
const GRADIENT =
...
export function Hero({ block, config, basePath = "" }: BlockComponentProps<HeroContent>) {
  ...
  return (
    <header className="relative isolate overflow-hidden text-white" style={{ background: GRADIENT }}>
      <Aura />
      <BubbleField />
      ...
      {/* Vague de sortie : le dégradé se déverse dans la page. */}
